...
class RETrainer(BaseTrainer):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True,
            ...
            for epoch in range(1, self.args.num_epochs + 1):
                ...
                for batch in self.train_data:
                    ...
                    graph_distill_loss_homo = 0.0005 * (loss_logit_homo + loss_reg_homo)
    ...
    def test(self):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.test_data:
                    ...
                    # all_sample_vectors = np.concatenate((all_sample_vectors, x.detach().cpu().numpy()), axis=0)
                    # all_labels = np.concatenate((all_labels, labels.view(-1).detach().cpu().numpy().reshape(-1, 1)),
                    #                             axis=0)
                    all_text_features = np.concatenate((all_text_features, uni_l.detach().cpu().numpy()), axis=0)
                    all_visual_features = np.concatenate((all_visual_features, uni_v.detach().cpu().numpy()), axis=0)
                    # all_sample_vectors = np.concatenate((all_sample_vectors, x.detach().cpu().numpy()), axis=0)
                    # all_labels = np.concatenate((all_labels, labels.view(-1).detach().cpu().numpy().reshape(-1, 1)),
                    #                             axis=0)
                    pbar.update()
        ...
        # all_com_features = np.vstack((all_text_features, all_visual_features))
        # np.save('all_com_features.npy',  all_com_features)

        tsne = TSNE(n_components=2, perplexity=20, learning_rate=200, n_iter=900, random_state=42)
        ...
        # plt.savefig('plot_o+g.png')
        # plt.show()
        cmap = ListedColormap(['#a8e6cf', '#87CEEB'])
        #features_2d = tsne.fit_transform(all_text_features)
        text_features_2d = tsne.fit_transform(all_text_features)
        visual_features_2d = tsne.fit_transform(all_visual_features)
        plt.figure(figsize=(10, 8))
        plt.scatter(text_features_2d[:, 0], text_features_2d[:, 1], c='#a8e6cf', label='Text Features')
        plt.scatter(visual_features_2d[:, 0], visual_features_2d[:, 1], c='#87CEEB', label='Visual Features')
        plt.legend()
        plt.grid(False)
        ax = plt.gca()
        ax.set_xticks([])
        ax.set_yticks([])
        legend_labels = ['textual', 'visual']
        handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=cmap(0), markersize=10),
                   plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=cmap(1), markersize=10)]
