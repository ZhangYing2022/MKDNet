...
class HMNeTREModel(nn.Module):
    ...
    def forward(
        ...
        x = torch.cat([hidden_phrases,entity_hidden_state,
                       pic_original+pic_original_objects
                       ], dim=-1)
        ...
        #return loss_task, logits, x
        #return combined_loss, repr_l_low_p, repr_v_low_o, logits_l_low_p, logits_v_low_o, logits, x
        return combined_loss, logits, entity_hidden_state,  pic_original+pic_original_objects
...
class HMNeTNERModel(nn.Module):
    def __init__(self, num_labels,label_list, args):
    ...
    def forward(self, input_ids=None, attention_mask=None, input_ids_phrase=None,
        attention_mask_phrase=None, token_type_ids=None, labels=None, images=None,
                aux_imgs=None):
        # if self.args.use_prompt:
        ...
        bert_output = self.bert(input_ids=input_ids,
                                ...
                                token_type_ids=token_type_ids,
                                return_dict=True)
        last_hidden_state = bert_output['last_hidden_state']  # bsz, len, hidden
        hidden_text = self.dropout(last_hidden_state)  # bsz, len, hidden
        ...
        pic_ori_final = (pic_original + pic_ori_)  + (pic_original_objects + pic_ori_objects_)
        ...
        loss_task = -1 * self.crf(emissions, labels, mask=attention_mask.byte(), reduction='mean')
        combined_loss = loss_task + loss_l_low_p + (loss_recon_l + loss_sl_slr) * 0.1
